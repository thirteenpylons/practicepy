...
"""
Assess part 1, the function teaches_multiengine and teaches_instrument
...
def check_func1(package,module):
    """
    Checks that the functions teaches_multiengine, teaches_instrument work correctly
    ...
    """
    outp = HTMLizer()
    result = verifier.grade_func1(package,module,0,outp)
    if not result[0]:
        result = verifier.grade_func2(package,module,0,outp)
    if not result[0]:
        print("The teacher functions look correct.")
    return result[0]


if __name__ == '__main__':
    sys.exit(check_func1('auditor','endorsements.py'))