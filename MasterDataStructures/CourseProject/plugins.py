...
def mono(image, sepia=False):
    ...
    for row in range(h):
        for col in range(w):
            b = 0.3 * image[row][col].red + 0.6 * image[row][col].green + 0.1 * image[row][col].blue
            image[row][col].red = int(b)
            image[row][col].green = int(b)
            image[row][col].blue = int(b)
    
            if sepia:
                image[row][col].green = int(image[row][col].green * 0.6)
                image[row][col].blue = int(image[row][col].blue * 0.4)
...
def flip(image,vertical=False):
    ...
    if vertical:
        for r in range(h // 2):
            for c in range(w):
                x = image[r][c]
                image[r][c] = image[h-r-1][c]
                image[h-r-1][c] = x
    else:
        for c in range(w // 2):
            for r in range(h):
                x = image[r][c]
                image[r][c] = image[r][w-c-1]
                image[r][w-c-1] = x
...
def transpose(image):
    ...
    All plug-in functions must return True or False.  This function returns True 
    because it modifies the image. It transposes the image, swaping columns and rows.
